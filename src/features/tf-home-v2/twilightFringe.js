...
export async function createTwilightFringe(containerEl, options = {}) {
  ...
  console.log(
    `${LOG_PREFIX} Setting background color (transparent alpha) to:`,
    settings.backgroundColor
  )
  // Keep the chosen color but clear with transparent alpha so canvases blend correctly
  try {
    renderer.setClearColor(new THREE.Color(settings.backgroundColor), 0)
  } catch (_) {
    renderer.setClearColor(settings.backgroundColor, 0)
  }
